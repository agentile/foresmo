...
class Foresmo_Hashing {
    ...
    public function hashPassword($password)
    {
        ...
        if (CRYPT_BLOWFISH == 1 && !$this->portable_hashes) {
            $this->hash_type = 'BLOWFISH';
            $random = $this->_getRandomBytes(16);
            $this->salt = $this->_genSaltBlowfish($random);
        ...
        if (CRYPT_EXT_DES == 1 && !$this->portable_hashes) {
            $this->hash_type = 'EXT_DES';
            if (strlen($random) < 3)
                $random = $this->_getRandomBytes(3);
            $this->salt = $this->_genSaltExtended($random);
            $hash = crypt($password, $this->salt);
            if (strlen($hash) == 20)
                return $hash;
        }
        ...
        $this->hash_type = 'MD5';
        $this->salt = $this->_genSaltPrivate($random);
        $hash = $this->_cryptPrivate($password, $this->salt);
        if (strlen($hash) == 34) {
            return $hash;
        }
